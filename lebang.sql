...
CREATE TABLE `task` (
  ...
  `procedure_num` tinyint(2) unsigned NOT NULL DEFAULT 5 COMMENT '任务步骤数',
...
CREATE TABLE `user_task` (
  ...
  `app_id` int(10) unsigned DEFAULT NULL COMMENT '用户来源app，冗余',
  `app_user_id` varchar(32) DEFAULT NULL COMMENT '用户在来源app中user_id，冗余',
  `task_id` bigint(20) unsigned NOT NULL COMMENT '任务id',
  `province_id` bigint(20) unsigned DEFAULT NULL COMMENT '省份id，冗余',
  `city_id` bigint(20) unsigned DEFAULT NULL COMMENT '城市id',
  `note` varchar(200) NOT NULL COMMENT '用户完成任务留言',
  `images` varchar(1000) DEFAULT NULL COMMENT '用户完成任务图片',
  `status` tinyint(2) unsigned NOT NULL COMMENT '任务进度',
  `task_end_time` int(10) unsigned NOT NULL DEFAULT 2147483647 COMMENT '任务结束时间，冗余',
